# Plan LinkedIn hasta la ponencia (13 oct 2026)

Objetivo: llegar al 13 de octubre posicionado en "estrategia antes que herramientas" (GTM con criterio), con el cuándo como idea insignia, sin destripar el caso de la ponencia.
Audiencia: dirección general / comercial B2B en España. Horario: CEST.
Nota: la hora importa mucho menos que la primera línea.

## Semana del 24 sep (preparación)

JUE 24 - DOM 27  solo interacción (20 min/día) + revisar titular y extracto con /li-profile antes de que llegue tráfico

## Semana del 28 sep

LUN 28  solo interacción
MAR 29  8:30  OPINIÓN   #17 Antes y ahora    - Si hacer ya casi no cuesta, lo que cuesta es decidir bien (draft-00, listo)
MIÉ 30  solo interacción
JUE 1   8:30  HISTORIA  #1  Llevar la contraria - Un «no» de marzo no vale lo mismo en octubre; momentum = estado del negocio (draft-01). Imagen: foto frente a película
VIE 2   solo interacción

## Semana del 5 oct

LUN 5   solo interacción
MAR 6   8:30  OFERTA    anuncio              - La ponencia del 13 (draft-02). Post de 2 imágenes: 1) cartel oficial de Day One (versión con MI CHARLA en la tarjeta de Ángel), 2) imagenes/2026-10-06_anuncio-dayone.png. Primer comentario: https://www.walead.ai/day-one Day One de WaLead, Madrid. Se puede nombrar Culligan y WaLead. Teatro Sofía, 13:00, walead.ai/day-one. Enlace en el primer comentario.
MIÉ 7   solo interacción
JUE 8   8:30  OPINIÓN   #11 Mito             - Más rápido no es más a tiempo (draft-03). Imagen: conversación genérica agente / cliente
VIE 9   solo interacción

## Semana del 12 oct

LUN 12  8:30  ENSEÑAR   #21 Valor directo    - Ofertas de empleo como señal (draft-04). Imagen: esquema señal → decisión
MAR 13  PONENCIA. Opcional: foto + 2 líneas desde el evento, sin texto largo
MIÉ 14  8:30  HISTORIA  resumen              - Qué preguntó la gente y qué me llevo. Lo más parecido a "prueba" que tenemos, porque no se pueden dar cifras internas.

Guardado para después del evento: "Una cuenta no deja de cambiar cuando firma" (entrar / crecer / proteger).

## Interacción

Ver `linkedin/guia.md`, sección 1 (lista de perfiles) y sección 2 (rutina diaria).
